...
@jit
def get_mass_and_dmdv(v, m_i=0.0, m_f=1.0, v_c=0.0, v_s=1.0):
    """
    Returns m(v) and dm/dv for the SPECS.md preferred profile:

        m(v) = m_i + (m_f - m_i)/2 * (1 + tanh((v - v_c) / v_s))

    Defaults correspond to vacuum-to-BTZ collapse: m(-inf)=0, m(+inf)=1.

    Parameters
    ----------
    v    : advanced time coordinate
    m_i  : initial (early-time) mass
    m_f  : final (late-time) mass
    v_c  : shell center in v
    v_s  : shell thickness in v
    """
    z = (v - v_c) / v_s
    t = jnp.tanh(z)
    m = m_i + (m_f - m_i) / 2.0 * (1.0 + t)
    dm_dv = (m_f - m_i) / 2.0 / v_s * (1.0 - t**2)
    return m, dm_dv


@jit
def get_derivs(state, lam, m_i=0.0, m_f=1.0, v_c=0.0, v_s=1.0):
    """
    ...
    m, dm_dv = get_mass_and_dmdv(v, m_i=m_i, m_f=m_f, v_c=v_c, v_s=v_s)
    f = r**2 - m
    df_dr = 2 * r
    df_dv = -dm_dv
...
@jit
def ds_dlambda(state, m_i=0.0, m_f=1.0, v_c=0.0, v_s=1.0):
    """
    Proper-length element for spacelike curve in Vaidya-AdS:
    ds = sqrt(-f dv^2 + 2 dv dr + r^2 dx^2) dλ
    """
    v, r, x, dv, dr, dx = state
    m, _ = get_mass_and_dmdv(v, m_i=m_i, m_f=m_f, v_c=v_c, v_s=v_s)
    f = r**2 - m
    inside = -f * dv**2 + 2.0 * dv * dr + (r**2) * dx**2
    return jnp.sqrt(jnp.maximum(inside, 0.0))


def speed_stats(traj, m_i=0.0, m_f=1.0, v_c=0.0, v_s=1.0):
    sdot = np.array(
        jax.vmap(lambda s: ds_dlambda(s, m_i=m_i, m_f=m_f, v_c=v_c, v_s=v_s))(traj)
    )
    return {
        "min": float(sdot.min()),
...
def lengths_vs_rstar(rstars, v0, n_steps=40000, dt=0.002, r_cut=200.0,
                     m_i=0.0, m_f=1.0, v_c=0.0, v_s=1.0):
    """
    ...
    m_i, m_f, v_c, v_s : float
        Mass profile parameters (see get_mass_and_dmdv).
    ...
    for r_star in rstars:
        traj = integrate_geodesic(
            r_star, v0, n_steps=int(n_steps), dt=float(dt),
...
def geodesic_length_from_traj(traj, dt, r_cut=200.0, m_i=0.0, m_f=1.0, v_c=0.0, v_s=1.0):
    """
    ...
    seg = traj[: hit + 1]
    sdot = jax.vmap(lambda s: ds_dlambda(s, m_i=m_i, m_f=m_f, v_c=v_c, v_s=v_s))(seg)
...
def length_profile_vs_x(traj, dt, r_cut=200.0, m_i=0.0, m_f=1.0, v_c=0.0, v_s=1.0):
    """
...
@jit
def rk4_step(state, dt, m_i=0.0, m_f=1.0, v_c=0.0, v_s=1.0):
    k1 = get_derivs(state, 0.0, m_i=m_i, m_f=m_f, v_c=v_c, v_s=v_s)
    k2 = get_derivs(state + 0.5 * dt * k1, 0.0, m_i=m_i, m_f=m_f, v_c=v_c, v_s=v_s)
    k3 = get_derivs(state + 0.5 * dt * k2, 0.0, m_i=m_i, m_f=m_f, v_c=v_c, v_s=v_s)
    k4 = get_derivs(state + dt * k3, 0.0, m_i=m_i, m_f=m_f, v_c=v_c, v_s=v_s)
    return state + (dt / 6.0) * (k1 + 2 * k2 + 2 * k3 + k4)


@partial(jax.jit, static_argnames=("n_steps",))
def integrate_geodesic(r_star, v0, n_steps=2000, dt=0.005, m_i=0.0, m_f=1.0, v_c=0.0, v_s=1.0):
    """
    ...
    def step_fn(state, _):
        new_state = rk4_step(state, dt, m_i=m_i, m_f=m_f, v_c=v_c, v_s=v_s)
